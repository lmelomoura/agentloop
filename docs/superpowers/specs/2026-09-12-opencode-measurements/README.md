...
| um turno tem um passo por chamada ao modelo: `step_finish.reason` é `tool-calls` entre ferramentas e `stop` no último; **excepto na compactação** (38: a CLI fecha o passo do resumo com `stop` e continua); os tokens e o custo são **por passo**, somados dão o run; o fim do run é o fim do processo (exit 0) | 02, 03, 05 |
...
| `38-compaction-continues.jsonl`, `38-compaction-continues.meta.txt`, `38-compaction-continues.stderr.txt` | observado numa análise real de 51 min (não numa sonda): a CLI **compacta a sessão sozinha** aos ~167k tokens (um passo próprio cujo `text` é o resumo do modelo, fechado com `step_finish.reason: stop`), injeta um `text` sintético (`part.synthetic: true`, `metadata.compaction_continue: true`, "Continue if you have next steps…") e o modelo continua por mais 35 min; e o provider fechou o socket duas vezes (`level=ERROR message="stream error"` no stderr), a CLI repetiu e o run acabou em `RUN COMPLETE` | `stop` **não é sempre o fim do run**: o normalizador passa a reabrir o run quando a CLI continua depois de um result (o último result é o do fim, com todos os tokens e custos); o `stream error` recuperado sai do stderr (a contagem fica no tick.log) e um run terminado deixa de ser `warning` por ele |
| `39-sandbox-parent-is-a-file.sh`, `39-sandbox-parent-is-a-file.out.txt`, `39-sandbox-parent-is-a-file.meta.txt` | depois de uma análise `deep` fechar `capped` com 231 verificações desistidas (2026-09-26): um repositório com três worktrees dispostos como os run dirs do motor, todo o `XDG_*` num rascunho, `opencode debug config` como sonda (arranca o projecto, não chama modelo nenhum) | o OpenCode guarda cada worktree onde arranca em `project.sandboxes` e verifica-os todos em cada arranque: um sandbox que desapareceu (ENOENT) é tolerado e podado, mas um cujo **pai é um ficheiro** (ENOTDIR) faz **todo** o arranque do projecto sair com rc 1, antes de haver sessão, com `BadResource: FileSystem.access (<sandbox>)`, em qualquer worktree, o checkout principal incluído; apagar o ficheiro cura. Confirmado também sobre um clone APFS da base real do operador, antes (rc 1) e depois (rc 0, sandboxes podados) da limpeza |
